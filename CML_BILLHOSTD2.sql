...
DROP PROCEDURE IF EXISTS CML_BILLHOSTD2;

DELIMITER $$

CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLHOSTD2 (IN IN_organizationId varchar(30),
...
cur_order_loop:
  LOOP
    ...
    /*begin
    DROP TABLE IF EXISTS TMP_SUM_BIL_HANDLING ;
    CREATE TEMPORARY TABLE TMP_SUM_BIL_HANDLING 
    (tms_organizationId varchar (255),
    tms_warehouseId varchar (255),
    tms_customerId varchar (255),
    tms_orderNo varchar (255),
    tms_orderType varchar (255),
    tms_soReference1 varchar (255),
    tms_soReference3 varchar (255),
    tms_tariffMasterId varchar (255),
    tms_qtyChargeEA varchar (255),
    tms_qtyChargeCS varchar (255),
    tms_qtyChargeIP varchar (255),
    tms_qtyChargePL varchar (255),
    tms_qtyChargeCBM varchar (255),
    tms_qtyChargeNettWeight varchar (255),
    tms_qtyChargeGrossWeight varchar (255),
    tms_qtyChargeMetricTon varchar (255));*/

    CASE
      WHEN od_tariffMasterId IN ('BIL01043') THEN BEGIN
          /*INSERT INTO TMP_SUM_BIL_HANDLING*/
          DECLARE summary_done boolean DEFAULT FALSE;
          DECLARE sum_orderno CURSOR FOR
          SELECT
            organizationId,
            warehouseId,
            customerId,
            ...
            SUM(qtyChargeCBM),
            SUM(qtyChargeTotDO),
            SUM(qtyChargeTotLine),
            SUM(qtyChargeNettWeight),
            SUM(qtyChargeGrossWeight),
            SUM(qtyChargeMetricTon)
          FROM (SELECT
              IFNULL(CAST(doh.organizationId AS char), '') AS organizationId,
              IFNULL(CAST(doh.orderNo AS char), '') AS orderNo,
              IFNULL(CAST(doh.soReference1 AS char(255)), '') AS soReference1,
              ...
              IFNULL(CAST(aad.qtyShipped_each AS char(255)), 0) AS qtyShipped_each,
              IFNULL(CAST(aad.uom AS char(255)), '') AS uom,
              CASE WHEN bpdCS.qty = 0 THEN 1 WHEN bpdCS.qty IS NULL THEN 1 ELSE IFNULL(CAST(bpdCS.qty AS char(255)), 0) END AS bpdCS,
              CASE WHEN bpdPL.qty = 0 THEN 1 WHEN bpdPL.qty IS NULL THEN 1 ELSE IFNULL(CAST(bpdIP.qty AS char(255)), 0) END AS bpdIP,
              CASE WHEN bpdIP.qty = 0 THEN 1 WHEN bpdIP.qty IS NULL THEN 1 ELSE IFNULL(CAST(bpdPL.qty AS char(255)), 0) END AS bpdPL,
              CASE WHEN aad.customerId IN ('MAP') AND
                  ila.lotAtt04 IN ('SETA', 'SET A', 'SET_A', 'SET-A', 'NONSET', 'NON SET', 'NON_SET', 'NON-SET') AND
                  bsm.tariffMasterId LIKE '%PIECE%' THEN IFNULL(CAST(SUM(aad.qtyShipped_each / bpdEA.qty) AS char(255)), 0) ELSE IFNULL(CAST(SUM(aad.qtyShipped_each / bpdEA.qty) AS char(255)), 0) END AS qtyChargeEA,
              CASE WHEN aad.customerId IN ('MAP') AND
                  ila.lotAtt04 IN ('SETA', 'SET A', 'SET_A', 'SET-A', 'NONSET', 'NON SET', 'NON_SET', 'NON-SET') AND
                  bsm.tariffMasterId NOT LIKE '%PIECE%' THEN IFNULL(CAST(SUM(aad.qtyShipped_each * bs.cube) AS char(255)), 0) ELSE IFNULL(CAST(SUM(aad.qtyShipped_each * bs.cube) AS char(255)), 0) END AS qtyChargeCBM,
              CASE WHEN bpdCS.qty = 0 THEN IFNULL(CAST(aad.qtyShipped_each AS char(255)), 0) WHEN bpdCS.qty IS NULL THEN IFNULL(CAST(aad.qtyShipped_each AS char(255)), 0) ELSE IFNULL(CAST(CEIL(SUM(aad.qtyShipped_each / bpdCS.qty)) AS char(255)), 0) END AS qtyChargeCS,
              CASE WHEN bpdIP.qty = 0 THEN IFNULL(CAST(aad.qtyShipped_each AS char(255)), 0) WHEN bpdIP.qty IS NULL THEN IFNULL(CAST(aad.qtyShipped_each AS char(255)), 0) ELSE IFNULL(CAST(CEIL(SUM(aad.qtyShipped_each / bpdIP.qty)) AS char(255)), 0) END AS qtyChargeIP,
              CASE WHEN bpdPL.qty = 0 THEN IFNULL(CAST(aad.qtyShipped_each AS char(255)), 0) WHEN bpdPL.qty IS NULL THEN IFNULL(CAST(aad.qtyShipped_each AS char(255)), 0) ELSE IFNULL(CAST(CEIL(SUM(aad.qtyShipped_each / bpdPL.qty)) AS char(255)), 0) END AS qtyChargePL,
              IFNULL(CAST(COUNT(doh.orderNo) AS char(255)), 0) AS qtyChargeTotDO,
              IFNULL(CAST(COUNT(aad.orderLineNo) AS char(255)), 0) AS qtyChargeTotLine,
              IFNULL(CAST((SUM(aad.qtyShipped_each * bs.cube)) AS char(255)), 0) AS totalCube,
              IFNULL(CAST(aad.editTime AS char(255)), '') AS editTime,
            ...
            FROM ACT_ALLOCATION_DETAILS aad
              ...
              LEFT JOIN BSM_CODE BT
                ...
                AND BT.codeType = 'BILLING_TRANSACTION_CATEGORY'
                AND BT.outerCode = ila.lotAtt07
            WHERE aad.customerId = 'TUMI'
            AND aad.warehouseId = 'CBT02'
            AND doh.orderNo = 'TUMI_ORDERNO00000004'
            -- AND bsm.tariffMasterId = IN_tariffMaster
            -- AND DATE_FORMAT(aad.shipmentTime, '%Y-%m-%d') >= '2023-09-10'
            --  AND DATE_FORMAT(aad.shipmentTime, '%Y-%m-%d') <= '2023-09-12'
            AND aad.Status IN ('99', '80')
        ...
        sum_order_loop:
          LOOP
            ...
            sum_qtyChargeCBM,
            sum_qtyChargeTotDO,
            sum_qtyChargeTotLine,
            sum_qtyChargeNettWeight,
            sum_qtyChargeGrossWeight,
            sum_qtyChargeMetricTon;
            IF order_done = 1 THEN

              SET summary_done = 0;
              LEAVE sum_order_loop;
            END IF;
          --     SELECT
          --       CONCAT('** 

          BLOCK1: /*get amount summary by order*/
            BEGIN
              DECLARE sum_Tariff CURSOR FOR
              SELECT DISTINCT
                bsm.organizationId,
                bsm.warehouseId,
                bsm.customerId,
            ...
            getTariff2:
              LOOP
                FETCH sum_Tariff INTO R_ORGANIZATIONID, R_WAREHOUSEID, R_CUSTOMERID, R_BILLINGDAY, R_TARIFFID, R_TARIFFLINENO, R_TARIFFCLASSNO, R_CHARGECATEGORY, R_CHARGETYPE, R_descrC, R_docType,
                ...
                IF tariff2_done THEN
                  SET tariff2_done = FALSE;
                  LEAVE getTariff2;
                END IF;
                ...
                END IF; tdak dipakai untuk yang sum 24/06/24*/

                IF (RTRIM(LTRIM(sum_orderType)) = RTRIM(LTRIM(R_docType))) THEN
                  /*AND (RTRIM(LTRIM(od_Billtranctg)) = RTRIM(LTRIM(R_BILLINGTRANCATEGORY)))*/
                  ...
                  IF (R_ratebase = 'CUBIC') THEN
                    SET R_RESULTQTYCHARGE = sum_qtyChargeCBM;
                  ELSEIF (R_ratebase = 'M2') THEN
                    SET R_RESULTQTYCHARGE = sum_qtyChargeEA;
                  ELSEIF (R_ratebase = 'IP') THEN
                    SET R_RESULTQTYCHARGE = sum_qtyChargeIP;
                  ...
                  ELSEIF (R_ratebase = 'DO') THEN
                    SET R_RESULTQTYCHARGE = CEILING(sum_qtyChargeEA / R_CLASSTO);
                  ELSEIF (R_ratebase = 'PALLET') THEN
                    SET R_RESULTQTYCHARGE = sum_qtyChargePL;
                  ELSEIF (R_ratebase = 'NETWEIGHT') THEN
                    SET R_RESULTQTYCHARGE = sum_qtyChargeNettWeight;
                  ELSEIF (R_ratebase = 'GW') THEN
                    SET R_RESULTQTYCHARGE = sum_qtyChargeGrossWeight; /*additional nettweight Gross Weight ABYuhuu*/
                  ELSEIF (R_ratebase = 'MT') THEN
                    SET R_RESULTQTYCHARGE = sum_qtyChargeMetricTon;/*additional metric ton 29042024*/
                  ...
                  , ediErrorCode2, ediErrorMessage2, billingTranCategory, orderType, containerType, containerSize)
                    SELECT
                      sum_organizationId AS organizationId,
                      sum_warehouseId AS warehouseId,
                      ...
                      R_rateBase,
                      R_rateperunit,
                      od_qtyShipped_each qtyShipped_each,
                      '' uom,
      ...
      WHEN od_tariffMasterId NOT IN ('BIL01043') THEN BEGIN
    ...
    END CASE; /*endded condition tariff masterid BIL01043*/
