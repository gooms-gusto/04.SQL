 SELECT DISTINCT
    ...
    bsm.warehouseId,
    bsm.CUSTOMERID,
    CAST(EXTRACT(MONTH FROM DATE (bth.billingdate)) AS STRING) as R_BILLINGDAY,
    ...
    btr.cost,
    btd.billingParty,
    FORMAT_DATE('%Y/%m/%d',CONCAT(CAST(EXTRACT(YEAR FROM DATE (CURRENT_DATE())) AS STRING), '-', CAST(EXTRACT(MONTH FROM DATE (CURRENT_DATE())) AS STRING), '-',  CAST(EXTRACT(MONTH FROM DATE (bth.billingdate)) AS STRING))) AS R_BILLINGDATE,
    FORMAT_DATE('%Y/%m/%d',DATE_ADD(DATE_ADD(FORMAT_DATE('%Y/%m/%d',CONCAT(CAST(EXTRACT(YEAR FROM DATE (CURRENT_DATE())) AS STRING), '-', CAST(EXTRACT(MONTH FROM DATE (CURRENT_DATE())) AS STRING), '-',  CAST(EXTRACT(MONTH FROM DATE (bth.billingdate)) AS STRING))), INTERVAL -1 MONTH), INTERVAL -1 DAY)) AS R_OPDATE
    ,FORMAT_DATE('%Y/%m/%d',DATE_ADD(FORMAT_DATE('%Y/%m/%d',CONCAT(CAST(EXTRACT(YEAR FROM DATE (CURRENT_DATE())) AS STRING), '-', CAST(EXTRACT(MONTH FROM DATE (CURRENT_DATE())) AS STRING), '-',  CAST(EXTRACT(MONTH FROM DATE (bth.billingdate)) AS STRING))), INTERVAL -1 MONTH)) AS R_FMDATE
    ,FORMAT_DATE('%Y/%m/%d',DATE_ADD(FORMAT_DATE('%Y/%m/%d',CONCAT(CAST(EXTRACT(YEAR FROM DATE (CURRENT_DATE())) AS STRING), '-', CAST(EXTRACT(MONTH FROM DATE (CURRENT_DATE())) AS STRING), '-',  CAST(EXTRACT(MONTH FROM DATE (bth.billingdate)) AS STRING))), INTERVAL -1 DAY)) AS R_TODATE
  
    ,DATEDIFF(DATE_ADD(FORMAT_DATE('%Y/%m/%d',CONCAT(CAST(EXTRACT(YEAR FROM DATE (CURRENT_DATE())) AS STRING), '-', CAST(EXTRACT(MONTH FROM DATE (CURRENT_DATE())) AS STRING), '-',  CAST(EXTRACT(MONTH FROM DATE (bth.billingdate)) AS STRING))), INTERVAL -1 DAY), FORMAT_DATE( '%Y/%m/%d',DATE_ADD(FORMAT_DATE('%Y/%m/%d',CONCAT(CAST(EXTRACT(YEAR FROM DATE (CURRENT_DATE())) AS STRING), '-', CAST(EXTRACT(MONTH FROM DATE (CURRENT_DATE())) AS STRING), '-',  CAST(EXTRACT(MONTH FROM DATE (bth.billingdate)) AS STRING))), INTERVAL -1 MONTH))) + 1 AS R_Days
...
INNER JOIN (
  ...
  AND tbs.StockDate >= FORMAT_DATE('%Y/%m/%d',DATE_ADD(FORMAT_DATE(CONCAT(CAST(EXTRACT(YEAR FROM DATE (CURRENT_DATE())) AS STRING), '-', CAST(EXTRACT(MONTH FROM DATE (CURRENT_DATE())) AS STRING), '-',  CAST(EXTRACT(MONTH FROM DATE (bth.billingdate)) AS STRING)), '%Y/%m/%d'), INTERVAL -1 MONTH))
  AND tbs.StockDate <= FORMAT_DATE('%Y/%m/%d',DATE_ADD(FORMAT_DATE(CONCAT(CAST(EXTRACT(YEAR FROM DATE (CURRENT_DATE())) AS STRING), '-', CAST(EXTRACT(MONTH FROM DATE (CURRENT_DATE())) AS STRING), '-',  CAST(EXTRACT(MONTH FROM DATE (bth.billingdate)) AS STRING)), '%Y/%m/%d'), INTERVAL -1 DAY))
    )
 ...
 -- AND bsm.customerId IN('API','ADS','PPG','MAP','GYVTL','LTL','HPK','ITOCHU','GCM','GYI','YFI','BCAFIN','BCA','RBFOOD','FFI','GMC','AGM','GYI','CERESSBY','NLDCSBY','JCISBY','GCMSBY','DNNSBY','HPKSBY','GMCSBY','AID_MDN','GCM_MDN','JJCHI_MDN','PT.ITT_MDN','NLDC','TRINITY','DKJ_SMG','PLB-LTL','ECCOSBY','TMB_SMG','SSISBY','UNZA','SOGOOD_SMG','WON_SMG','PPT_SMG','GMC_SMG','CERESSMG','GMC_SMG')
  AND bth.effectiveFrom <= FORMAT_DATE(CURDATE(), '%Y/%m/%d')
  AND bth.effectiveTo >= FORMAT_DATE(CURDATE(), '%Y/%m/%d')
