...
PROCEDURE CML_BILLVASSPECIALSTD_NW (IN IN_organizationId varchar(30),
  ...
  START TRANSACTION;
    ...
    BEGIN
      ...
      IF v_row_count = 0 THEN
        ...
        SET p_message = CONCAT('No billing data found for Vas Special: ', IN_trans_no,
      ...
      ELSE
        ...
        SET p_message = CONCAT('VasSpecial billing processed successfully. ',
...
--
-- Create procedure `CML_BILLTRFCSSTD_NW`
--
CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLTRFCSSTD_NW (IN IN_organizationId varchar(30),
IN IN_warehouseId varchar(30),
IN IN_USERID varchar(30),
IN IN_Language varchar(30),
IN IN_CustomerId varchar(30),
  ...
  START TRANSACTION;
    ...
    BEGIN
      ...
      SELECT
        ...
        trf.warehouseId,
        trf.customerId,
        CONCAT('rate range classFrom: ', CAST(btr.classFrom AS SIGNED), '-', 'classTo:', CAST(btr.classTo AS SIGNED)) AS codeDescr,
        trf.tdocNo,
        trf.editTime,
        trf.tdocType,
        trf.chargeCategory,
        trf.chargeType,
        trf.qtyCSAll AS qtyChargeBilling,
        btr.rate,
        1 ratePerUnit,
        0 incomeTaxRate,
        0 cost,
      ...
      FROM (SELECT
          ...
          trans.tdocNo,
          trans.tdocType,
          SUM(trans.qtyCSAll) AS qtyCSAll,
          bil.tariffLineNo,
          bil.tariffMasterId,
          bil.tariffId,
          trans.editTime,
        ...
        FROM (SELECT
            ...
            dth.warehouseId,
            dtd.tdocNo,
            CEIL(SUM(dtd.toQty / bp.qty)) AS qtyCSAll,
            dtd.toSku,
            dth.tdocType,
            bsm.tariffMasterId,
            dth.editTime
          FROM DOC_TRANSFER_HEADER dth
            ...
            INNER JOIN BAS_PACKAGE_DETAILS bp
              ON dth.organizationId = bp.organizationId
              AND bsm.packId = bp.packId
              AND dth.customerId = bp.customerId
              AND packUom = 'CS'
          WHERE dth.organizationId = 'OJV_CML'
          AND
          ...
          AND zbcc.active = 'Y'
          AND zbccd.active = 'Y'
          AND zbccd.spName = 'CML_BILLTRFCSSTD'
          AND dth.status = '99'
      ...
      WHERE trf.organizationId = IN_organizationId
      AND trf.warehouseId = IN_warehouseId
      AND trf.customerId = IN_CustomerId
      AND trf.tdocNo = IN_trans_no
      ...
      containerType, containerSize)
        SELECT
          ...
          warehouseId,
          CONCAT(v_billing_summary_id, '*', LPAD(auto_sequence(), 3, '0')) AS billingSummaryId,
          DATE(editTime) AS billingFromDate,
          DATE(editTime) AS billingToDate,
          customerId,
          '' sku,
          '' lotNum,
          '' traceId,
      ...
      IF v_row_count = 0 THEN
        -- No data found, but this might be expected
        SET p_message = CONCAT('No billing data found for Vas SO: ', IN_trans_no,
        ', Customer: ', IN_CustomerId);
        SET p_success_flag = 'W'; -- Warning flag
        SET p_record_count = 0;
        COMMIT; -- Commit even if no rows
      ...
      ELSE
        ...
        -- Set success output parameters
        SET p_success_flag = 'Y';
        SET p_message = CONCAT('TransferCS Range billing processed successfully. ',
        'Billing Summary ID: ', v_billing_summary_id,
        '. Records: ', v_row_count,
        '. Execution time: ', v_execution_time, ' seconds');
        SET p_record_count = v_row_count;

        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLTRFCSSTD_NW', IN_trans_no, 'no error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
...
--
-- Create procedure `Z_SP_ProcessBillingCML_BILLTRFCSSTD`
--
CREATE
DEFINER = 'mysql.sys'@'%'
PROCEDURE Z_SP_ProcessBillingCML_BILLTRFCSSTD ()
    SP:
  BEGIN
    -- Deklarasi variabel untuk menyimpan data dari cursor
    DECLARE v_organizationId varchar(20);
    ...
    AND dah.editTime > getBillFMDate(25)
    AND NOT EXISTS (SELECT
        1
      FROM BIL_SUMMARY bs
      WHERE bs.organizationId = 'OJV_CML'
    ...
    LOOP
      ...
      SET @IN_CustomerId = v_customerId;
      SET @IN_trans_no = v_trans_no;
      CALL CML_BILLTRFCSSTD_NW(@IN_organizationId, @IN_warehouseId, @IN_USERID, @IN_Language, @IN_CustomerId, @IN_trans_no, @p_success_flag, @p_message, @p_record_count);
...
CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLTRFBAGGINGSTD_NW (IN IN_organizationId varchar(30),
IN IN_warehouseId varchar(30),
IN IN_USERID varchar(30),
IN IN_Language varchar(30),
IN IN_CustomerId varchar(30),
  ...
  START TRANSACTION;
    ...
    BEGIN
      ...
      ELSE
        ...
        -- Set success output parameters
        SET p_success_flag = 'Y';
        SET p_message = CONCAT('TransferBagging billing processed successfully. ',
        'Billing Summary ID: ', v_billing_summary_id,
        '. Records: ', v_row_count,
        '. Execution time: ', v_execution_time, ' seconds');
        SET p_record_count = v_row_count;

        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLTRFBAGGINGSTD_NW', IN_trans_no, 'no error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
...
CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLSOVASSTD_NW (IN IN_organizationId varchar(30),
IN IN_warehouseId varchar(30),
IN IN_USERID varchar(30),
IN IN_Language varchar(30),
IN IN_CustomerId varchar(30),
  ...
  START TRANSACTION;
    ...
    BEGIN
      ...
      SELECT
        ...
        bil.chargeCategory,
        bil.chargeType,
        CASE WHEN doh.customerId = 'PPG' AND
            doh.warehouseId = 'CBT01' AND
            dov.vasType IN ('PW', 'SWR', 'WR') THEN 0 ELSE bil.rate END AS rate,
        'QUANTITY' AS ratebase,
        0 cost,
        1 AS rateperunit,
        bil.udf01,
      ...
      FROM DOC_ORDER_VAS dov
        ...
        INNER JOIN DOC_ORDER_DETAILS dod
          ON dov.organizationId = dod.organizationId
        ...
        INNER JOIN (SELECT
          ...
          AND bil.warehouseId = bsm.warehouseId
          AND bil.tariffMasterId = bsm.tariffMasterId
          AND bil.vasType = dov.vasType
        INNER JOIN BSM_CODE_ML bcm
          ON dov.organizationId = bcm.organizationId
          AND bcm.codeType = 'VAS_TYP'
          AND bcm.codeid = dov.vasType
          AND bcm.languageId = 'en'
        ...
        INNER JOIN Z_BAS_CUSTOMER_CUSTBILLING_DETAILS zbccd
          ON (zbcc.organizationId = zbccd.organizationId
          AND zbcc.lotatt01 = zbccd.idGroupSp)
      WHERE dov.organizationId = IN_organizationId
      AND dov.warehouseId = IN_warehouseId
      ...
      ELSE
        ...
        -- Set success output parameters
        SET p_success_flag = 'Y';
        SET p_message = CONCAT('VasSO billing processed successfully. ',
        'Billing Summary ID: ', v_billing_summary_id,
        '. Records: ', v_row_count,
        '. Execution time: ', v_execution_time, ' seconds');
        SET p_record_count = v_row_count;
...
PROCEDURE Z_SP_ProcessBillingCML_BILLSOVASSTD ()
  ...
  BEGIN
    ...
    AND DATE(dahu.closeTime) > getBillFMDate(25)
    AND zbcc.lotatt01 <> ''
    AND zbcc.active = 'Y'
    AND zbccd.active = 'Y'
    AND doh.soStatus IN ('99')
...
--
-- Create procedure `CML_BILLHOSTD_TYPE2_TRACEIDPERSO_NW`
--
CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLHOSTD_TYPE2_TRACEIDPERSO_NW (IN IN_organizationId varchar(30), IN IN_warehouseId varchar(30), IN IN_USERID varchar(30), IN IN_Language varchar(30), IN IN_CustomerId varchar(30), IN IN_trans_no varchar(30), OUT p_success_flag char(1), OUT p_message varchar(1000), OUT p_record_count int)
BEGIN
  -- =============================================
  ...
  START TRANSACTION;
    ...
    BEGIN
      ...
      SELECT
        ...
        '' editTime,
        '' lotNum,
        aad.traceId traceId,       -- khususon hitung pallet by picktotraceid per SO -- @AKBAR-@ITLINC -- 12.09.25
        aad.traceId pickToTraceId, -- khususon hitung pallet by picktotraceid per SO -- @AKBAR-@ITLINC -- 12.09.25
        aad.traceId dropId,        -- khususon hitung pallet by picktotraceid per SO -- @AKBAR-@ITLINC -- 12.09.25
        '' location,
        '' pickToLocation,
        '' allocationDetailsId,
        '' skuDescr1,
        ...
        CASE WHEN btd.ratebase = 'PALLET' THEN 1
          --                         WHEN btd.ratebase = 'DO' AND aad.customerId IN ('ECMAMAB2C') THEN  CEIL(SUM(aad.qty_each/btr.classTo))
          --                         WHEN btd.ratebase = 'DO' AND aad.customerId NOT IN ('ECMAMAB2C') THEN  COUNT(DISTINCT doh.orderNo)
          --  WHEN btd.ratebase = 'DID' THEN COUNT(DISTINCT aad.dropId)
          --                         WHEN btd.ratebase = 'DID1' AND aad.customerId IN('TMB_SMG') THEN COUNT(DISTINCT aad.dropId)
          ELSE 0 END AS qtyChargeBilling,
        -- Tariff details
        btr.rate,
        btd.ratebase,
        btr.tariffId,
      ...
      IF v_row_count = 0 THEN
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHOSTD_TYPE2_TRACEID_NW', IN_trans_no, 'error', NULL, NULL, p_success_flag, p_message, p_record_count, v_start_time, v_end_time);
      ELSE
        -- Calculate execution time
        SET v_end_time = NOW();
        SET v_execution_time = TIMESTAMPDIFF(MICROSECOND, v_start_time, v_end_time) / 1000000;
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, ' CML_BILLHOSTD_TYPE2_TRACEID_NW', IN_trans_no, 'no error', NULL, NULL, p_success_flag, p_message, p_record_count, v_start_time, v_end_time);
...
--
-- Create procedure `Z_SP_ProcessBillingCML_BILLHOTRACEIDPERSO`
--
CREATE
DEFINER = 'mysql.sys'@'%'
PROCEDURE Z_SP_ProcessBillingCML_BILLHOTRACEIDPERSO ()
    SP:
  BEGIN
    -- Deklarasi variabel untuk menyimpan data dari cursor
    DECLARE v_organizationId varchar(20);
    ...
    AND doh.orderType NOT IN ('FREE', 'KT', 'TROF', 'TTG', 'DS')
    --   doh.warehouseId='' AND
    AND doh.soStatus = '99'
    AND zbcc.lotatt01 <> ''
    AND zbcc.active = 'Y'
    AND zbccd.active = 'Y'
    AND zbccd.spName = 'CML_BILLHOSTD_TYPE2_TRACEIDPERSO_NW'
    AND DATE(doh.editTime) > getBillFMDate(25)
    AND NOT EXISTS (SELECT
        1
      FROM BIL_SUMMARY bs
      WHERE bs.organizationId = 'OJV_CML'
    ...
    LOOP
      ...
      SET @IN_CustomerId = v_customerId;
      SET @IN_trans_no = v_trans_no;
      CALL CML_BILLHOSTD_TYPE2_TRACEIDPERSO_NW(@IN_organizationId, @IN_warehouseId, @IN_USERID, @IN_Language, @IN_CustomerId, @IN_trans_no, @p_success_flag, @p_message, @p_record_count);
...
CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLHOSTD_TYPE2_NW (IN IN_organizationId varchar(30),
IN IN_warehouseId varchar(30),
IN IN_USERID varchar(30),
IN IN_Language varchar(30),
IN IN_CustomerId varchar(30),
  ...
  START TRANSACTION;
    ...
    BEGIN
      ...
      SELECT
        ...
        '' billtranctg,
        0 qtyChargeNettWeight,
        CASE WHEN btd.ratebase = 'DO' AND
            aad.customerId IN ('ECMAMAB2C') THEN CEIL(SUM(aad.qty_each / btr.classTo)) WHEN btd.ratebase = 'PALLET' AND
            aad.customerId IN ('ECMAMAB2C') THEN COUNT(DISTINCT (aad.pickToTraceId)) WHEN btd.ratebase = 'DO' AND
            aad.customerId NOT IN ('ECMAMAB2C') THEN COUNT(DISTINCT doh.orderNo) WHEN btd.ratebase = 'DID' THEN COUNT(DISTINCT aad.dropId) WHEN btd.ratebase = 'DID1' AND
            aad.customerId IN ('TMB_SMG') THEN COUNT(DISTINCT aad.dropId) ELSE 0 END AS qtyChargeBilling,
        -- Tariff details
        btr.rate,
        btd.ratebase,
        btr.tariffId,
      ...
      IF v_row_count = 0 THEN
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHOSTD2', IN_trans_no, 'error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
      ELSE
        -- Calculate execution time
        SET v_end_time = NOW();
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHOSTD2', IN_trans_no, 'no error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
...
--
-- Create procedure `CML_BILLHOSTDPL_NW`
--
CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLHOSTDPL_NW (IN IN_organizationId varchar(30),
IN IN_warehouseId varchar(30),
IN IN_USERID varchar(30),
IN IN_Language varchar(30),
IN IN_CustomerId varchar(30),
...
BEGIN
  -- =============================================
  -- Stored Procedure: CML_BILLHOSTDPL_NW
  -- Purpose: Process outbound billing for customers based on shipment transactions
  -- Author: [Akbar@IT-LINC]
  -- Date: [27.08.25]
  -- =============================================
  ...
  START TRANSACTION;
    ...
    BEGIN
      ...
      SELECT
        ...
        '' orderLineNo,
        '' SKU,
        DATE_FORMAT(MAX(aad.shipmentTime), '%Y-%m-%d') AS ShipmentTime,
        1 qty,
        1 qty_each,
        1 qtyShipped_each,
        'PL' uom,
        ...
        1 AS qtyChargeCBM,
        1 qtyChargeTotDO,
        1 AS qtyChargeTotLine,
        1 AS totalCube,

        MAX(doh.editTime) AS editTime,
        '' lotNum,
        '' traceId,
        aad.pickToTraceId,
        aad.pickToTraceId dropId,
        '' location,
        '' pickToLocation,
        '' allocationDetailsId,
        '' skuDescr1,
        ...
        '' AS lotAtt17,
        '' AS billtranctg,
        1 AS qtyChargeNettWeight,

        1 AS qtyChargeMetricTon,

        MAX(df.closeTime) AS closeTime,

        -- Calculate billing quantity based on rate base
        1 AS qtyChargeBilling,
      ...
      FROM ACT_ALLOCATION_DETAILS aad
        ...
        LEFT JOIN BSM_CODE BT
          ON BT.organizationId = 'OJV_CML'
          AND BT.codeType = 'BILLING_TRANSACTION_CATEGORY'
          AND BT.outerCode = aad.udf05 -- // UDF05 FOR PALLETIZE USED
        LEFT JOIN BAS_LOCATION bl
          ON bl.organizationId = aad.organizationId
          AND bl.warehouseId = aad.warehouseId
          AND bl.locationId = aad.location
      ...
      AND aad.warehouseId = IN_warehouseId
      AND aad.customerId = IN_CustomerId
      AND aad.pickToTraceId = IN_trans_no
      AND btd.ratebase = 'PALLET'
      AND aad.Status IN ('99', '80')
      AND doh.soStatus = '99'
      AND bth.effectiveFrom <= DATE_FORMAT(CURDATE(), '%Y-%m-%d')
      AND bth.effectiveTo >= DATE_FORMAT(CURDATE(), '%Y-%m-%d')
      ...
      IF v_row_count = 0 THEN
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHOSTDPL_NW', IN_trans_no, 'error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
      ELSE
        -- Calculate execution time
        SET v_end_time = NOW();
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHOSTDPL_NW', IN_trans_no, 'no error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
...
--
-- Create procedure `Z_SP_ProcessBillingCML_BILLHOSTDPL`
--
CREATE
DEFINER = 'mysql.sys'@'%'
PROCEDURE Z_SP_ProcessBillingCML_BILLHOSTDPL ()
    SP:
  BEGIN
    -- Deklarasi variabel untuk menyimpan data dari cursor
    DECLARE v_organizationId varchar(20);
    ...
    AND NOT EXISTS (SELECT
      ...
      FROM BIL_SUMMARY bs
      WHERE bs.organizationId = 'OJV_CML'
      AND bs.docNo = aad.pickToTraceId
      AND bs.warehouseId = doh.warehouseId
      AND bs.customerId = doh.customerId
      AND bs.chargeCategory = 'OB'
      AND bs.addTime >= DATE_SUB(CURDATE(), INTERVAL 2 MONTH))
    ...
    LOOP
      ...
      SET @IN_CustomerId = v_customerId;
      SET @IN_trans_no = v_trans_no;
      CALL CML_BILLHOSTDPL_NW(@IN_organizationId, @IN_warehouseId, @IN_USERID, @IN_Language, @IN_CustomerId, @IN_trans_no, @p_success_flag, @p_message, @p_record_count);
    ...
    SET zsbl.flag = 0,
        zsbl.changeTime = NOW()
    WHERE zsbl.spName = ' CML_BILLHOSTDPL_NW';
    -- Tutup cursor
    CLOSE billing_cursor;
...
--
-- Create procedure `CML_BILLHOSTDLS_NW`
--
CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLHOSTDLS_NW (IN IN_organizationId varchar(30),
IN IN_warehouseId varchar(30),
IN IN_USERID varchar(30),
IN IN_Language varchar(30),
IN IN_CustomerId varchar(30),
    ...
    BEGIN
      ...
      SELECT
        ...
        CASE WHEN btd.ratebase = 'CUBIC' THEN SUM(aad.qtyShipped_each * bs.cube) WHEN btd.ratebase = 'M2' THEN CASE WHEN aad.customerId = 'MAP' AND
                  ...
                  ila.lotAtt04 IN ('SETA', 'SET A', 'SET_A', 'SET-A', 'NONSET', 'NON SET', 'NON_SET', 'NON-SET') AND
                  bsm.tariffMasterId LIKE '%PIECE%' THEN SUM(aad.qtyShipped_each / NULLIF(bpdEA.qty, 0)) WHEN aad.customerId = 'PPG' AND
                  bsm.tariffMasterId = 'BIL00061KG' THEN SUM(aad.qtyShipped_each / NULLIF(bpdEA.qty, 0)) * NULLIF(bs.sku_group6, 0) WHEN aad.customerId = 'PPG' AND
                  bsm.tariffMasterId = 'BIL00061' THEN SUM(aad.qtyShipped_each / NULLIF(bpdEA.qty, 0)) ELSE SUM(aad.qtyShipped_each / NULLIF(bpdEA.qty, 0)) END WHEN btd.ratebase = 'QUANTITY' THEN CASE WHEN aad.customerId = 'MAP' AND
                  ila.lotAtt04 IN ('SETA', 'SET A', 'SET_A', 'SET-A', 'NONSET', 'NON SET', 'NON_SET', 'NON-SET') AND
      ...
      IF v_row_count = 0 THEN
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHOSTDLS_NW', IN_trans_no, 'error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
      ELSE
        -- Calculate execution time
        SET v_end_time = NOW();
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHOSTDLS_NW', IN_trans_no, 'no error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
...
--
-- Create procedure `Z_SP_ProcessBillingCML_BILLHOSTDLS`
--
CREATE
DEFINER = 'mysql.sys'@'%'
PROCEDURE Z_SP_ProcessBillingCML_BILLHOSTDLS ()
    SP:
  BEGIN
    -- Deklarasi variabel untuk menyimpan data dari cursor
    DECLARE v_organizationId varchar(20);
    ...
    AND zbcc.active = 'Y'
    AND zbccd.active = 'Y'
    AND zbccd.spName = 'CML_BILLHOSTDLS_NW'
    AND DATE(doh.editTime) > getBillFMDate(25)
    AND NOT EXISTS (SELECT
        1
      FROM BIL_SUMMARY bs
    ...
    LOOP
      ...
      SET @IN_CustomerId = v_customerId;
      SET @IN_trans_no = v_trans_no;
      CALL CML_BILLHOSTDLS_NW(@IN_organizationId, @IN_warehouseId, @IN_USERID, @IN_Language, @IN_CustomerId, @IN_trans_no, @p_success_flag, @p_message, @p_record_count);
...
CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLHOSTD_NW (IN IN_organizationId varchar(30),
IN IN_warehouseId varchar(30),
IN IN_USERID varchar(30),
IN IN_Language varchar(30),
IN IN_CustomerId varchar(30),
    ...
    BEGIN
      ...
      SELECT
        ...
        CASE WHEN btd.ratebase = 'CUBIC' THEN SUM(aad.qtyShipped_each * bs.cube) WHEN btd.ratebase = 'M2' THEN CASE WHEN aad.customerId = 'MAP' AND
                  ...
                  bsm.tariffMasterId LIKE '%PIECE%' THEN SUM(aad.qtyShipped_each / NULLIF(bpdEA.qty, 0)) ELSE SUM(aad.qtyShipped_each / NULLIF(bpdEA.qty, 0)) END WHEN btd.ratebase = 'LITER' THEN CASE WHEN aad.customerId = 'MAP' AND
                  ila.lotAtt04 IN ('SETA', 'SET A', 'SET_A', 'SET-A', 'NONSET', 'NON SET', 'NON_SET', 'NON-SET') AND
                  bsm.tariffMasterId LIKE '%PIECE%' THEN SUM(aad.qtyShipped_each / NULLIF(bpdEA.qty, 0)) WHEN aad.customerId = 'PPG' THEN SUM(aad.qtyShipped_each / NULLIF(bpdEA.qty, 0)) * NULLIF(bs.sku_group6, 0) ELSE SUM(aad.qtyShipped_each / NULLIF(bpdEA.qty, 0)) END WHEN btd.ratebase = 'QUANTITY' THEN CASE WHEN aad.customerId = 'MAP' AND
                  ila.lotAtt04 IN ('SETA', 'SET A', 'SET_A', 'SET-A', 'NONSET', 'NON SET', 'NON_SET', 'NON-SET') AND
                  bsm.tariffMasterId LIKE '%PIECE%' THEN SUM(aad.qtyShipped_each / NULLIF(bpdEA.qty, 0)) ELSE SUM(aad.qtyShipped_each / NULLIF(bpdEA.qty, 0)) END
          --  WHEN btd.ratebase = 'DO' THEN COUNT(DISTINCT doh.orderNo) * DO Ratebase cannot use this store procedure
          WHEN btd.ratebase = 'PALLET' THEN CEIL(SUM(aad.qtyShipped_each / NULLIF(bpdPL.qty, 0))) WHEN btd.ratebase = 'CASE' THEN CEIL(SUM(aad.qtyShipped_each / NULLIF(bpdCS.qty, 0))) WHEN btd.ratebase = 'NETWEIGHT' THEN SUM(aad.qtyShipped_each * bs.netWeight) WHEN btd.ratebase = 'GW' THEN CASE WHEN bpdEA.uomDescr IN ('G', 'GRAM', 'Gram') THEN SUM(aad.qtyShipped_each / 1000) WHEN bpdEA.uomDescr IN ('MT', 'Metric Ton', 'METRIC TON') THEN SUM(aad.qtyShipped_each * 1000) ELSE SUM(aad.qtyShipped_each * bs.grossWeight) END WHEN btd.ratebase = 'MT' THEN CASE WHEN aad.customerId LIKE '%ABC%' THEN SUM((aad.qtyShipped_each * bpdCS.qty) / 1000) WHEN aad.customerId IN ('ADASBY', 'CAI_MDN', 'CAI_SBY') THEN IFNULL(CAST(SUM((aad.qtyShipped_each * bs.netweight) / 1000) AS char(255)), 0) ELSE SUM(aad.qtyShipped_each / 1000) END ELSE 0 END AS qtyChargeBilling,
      ...
      FROM ACT_ALLOCATION_DETAILS aad
        ...
        LEFT JOIN BSM_CODE BT
          ON BT.organizationId = 'OJV_CML'
          AND BT.codeType = 'BILLING_TRANSACTION_CATEGORY'
          AND CASE WHEN aad.customerId = 'PPG' THEN BT.outerCode = aad.udf05 ELSE BT.outerCode = ila.lotAtt07 END
        LEFT JOIN BAS_LOCATION bl
          ON bl.organizationId = aad.organizationId
          AND bl.warehouseId = aad.warehouseId
          AND bl.locationId = aad.location
      ...
      OR btd.billingTranCategory = ''
      OR btd.billingTranCategory = BT.codeid
      OR btd.billingTranCategory = aad.udf05
      )
      AND btr.rate > 0
      AND NOT EXISTS (SELECT
          1
      ...
      ELSE
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHOSTD_NW', IN_trans_no, 'no error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
...
--
-- Create procedure `CML_BILLHOSTD_TYPE2_PICKPERSO_NW`
--
CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLHOSTD_TYPE2_PICKPERSO_NW (IN IN_organizationId varchar(30),
IN IN_warehouseId varchar(30),
IN IN_USERID varchar(30),
IN IN_Language varchar(30),
IN IN_CustomerId varchar(30),
  ...
  START TRANSACTION;
    ...
    BEGIN
      ...
      SELECT
        ...
        '' editTime,
        '' lotNum,
        aad.pickToTraceId traceId,       -- khususon hitung pallet by picktotraceid per SO -- @AKBAR-@ITLINC -- 12.09.25
        aad.pickToTraceId pickToTraceId, -- khususon hitung pallet by picktotraceid per SO -- @AKBAR-@ITLINC -- 12.09.25
        aad.pickToTraceId dropId,        -- khususon hitung pallet by picktotraceid per SO -- @AKBAR-@ITLINC -- 12.09.25
        '' location,
        '' pickToLocation,
        '' allocationDetailsId,
        '' skuDescr1,
        ...
        '' billtranctg,
        0 qtyChargeNettWeight,
        CASE WHEN btd.ratebase = 'PALLET' AND
            aad.customerId NOT IN ('YFI') THEN 1
          --                         WHEN btd.ratebase = 'DO' AND aad.customerId IN ('ECMAMAB2C') THEN  CEIL(SUM(aad.qty_each/btr.classTo))
          --                         WHEN btd.ratebase = 'DO' AND aad.customerId NOT IN ('ECMAMAB2C') THEN  COUNT(DISTINCT doh.orderNo)
          WHEN btd.ratebase = 'DID' THEN 1
          --                         WHEN btd.ratebase = 'DID1' AND aad.customerId IN('TMB_SMG') THEN COUNT(DISTINCT aad.dropId)
          ELSE 0 END AS qtyChargeBilling,
        -- Tariff details
        btr.rate,
      ...
      IF v_row_count = 0 THEN
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHOSTD_TYPE2_PP_NW', IN_trans_no, 'error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
      ELSE
        -- Calculate execution time
        SET v_end_time = NOW();
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHOSTD_TYPE2_PP_NW', IN_trans_no, 'no error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
...
--
-- Create procedure `Z_SP_ProcessBillingCML_BILLHOPICKPERSO`
--
CREATE
DEFINER = 'mysql.sys'@'%'
PROCEDURE Z_SP_ProcessBillingCML_BILLHOPICKPERSO ()
    SP:
  BEGIN
    -- Deklarasi variabel untuk menyimpan data dari cursor
    DECLARE v_organizationId varchar(20);
    ...
    AND zbcc.active = 'Y'
    AND zbccd.active = 'Y'
    AND zbccd.spName = 'CML_BILLHOSTD_TYPE2_PICKPERSO_NW'
    AND DATE(doh.editTime) > getBillFMDate(25)
    AND NOT EXISTS (SELECT
        1
      FROM BIL_SUMMARY bs
    ...
    LOOP
      ...
      SET @IN_CustomerId = v_customerId;
      SET @IN_trans_no = v_trans_no;
      CALL CML_BILLHOSTD_TYPE2_PICKPERSO_NW(@IN_organizationId, @IN_warehouseId, @IN_USERID, @IN_Language, @IN_CustomerId, @IN_trans_no, @p_success_flag, @p_message, @p_record_count);
...
CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLHISTD_TYPE2_NW (IN IN_organizationId varchar(30),
IN IN_warehouseId varchar(30),
IN IN_USERID varchar(30),
IN IN_Language varchar(30),
IN IN_CustomerId varchar(30),
  ...
  START TRANSACTION;
    ...
    BEGIN
      ...
      SELECT
        ...
        -- Calculate billing quantity based on rate base
        CASE WHEN btd.ratebase = 'MUID' THEN CEIL(COUNT(DISTINCT (atl.fmMuid)) / btr.classTo) WHEN btd.ratebase = 'DID' THEN CEIL(COUNT(DISTINCT (atl.fmMuid)) / btr.classTo) WHEN btd.ratebase = 'DID1' THEN CEIL(COUNT(DISTINCT (atl.fmMuid)) / btr.classTo) WHEN btd.ratebase = 'DO' THEN CEIL(COUNT(DISTINCT (dah.asnNo)) / btr.classTo) WHEN btd.ratebase = 'PALLET' THEN CEIL(COUNT(DISTINCT (atl.toId)) / btr.classTo) WHEN btd.ratebase = 'CASE' THEN CEIL(SUM(atl.toQty_Each / NULLIF(bpdCS.qty, 0))) ELSE 0 END AS qtyChargeBilling,
        0 AS qtyChargeTotDO,
      ...
      IF v_row_count = 0 THEN
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHISTD_TYPE2', IN_trans_no, 'error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
      ELSE
        -- Calculate execution time
        SET v_end_time = NOW();
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHISTD_TYPE2_NW', IN_trans_no, 'no error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
...
CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLHISTD_NW (IN IN_organizationId varchar(30),
IN IN_warehouseId varchar(30),
IN IN_USERID varchar(30),
IN IN_Language varchar(30),
IN IN_CustomerId varchar(30),
    ...
    BEGIN
      ...
      SELECT
        ...
        CASE WHEN btd.ratebase = 'CUBIC' THEN SUM(atl.toQty_Each * bs.cube) WHEN btd.ratebase = 'M2' THEN CASE WHEN atl.tocustomerId = 'MAP' AND
                  ...
                  ila.lotAtt04 IN ('SETA', 'SET A', 'SET_A', 'SET-A', 'NONSET', 'NON SET', 'NON_SET', 'NON-SET') AND
                  bsm.tariffMasterId LIKE '%PIECE%' THEN SUM(atl.toQty_Each / NULLIF(bpdEA.qty, 0)) WHEN atl.tocustomerId = 'PPG' THEN SUM(atl.toQty_Each / NULLIF(bpdEA.qty, 0)) * NULLIF(bs.sku_group6, 0) ELSE SUM(atl.toQty_Each / NULLIF(bpdEA.qty, 0)) END WHEN btd.ratebase = 'QUANTITY' THEN CASE WHEN atl.tocustomerId = 'MAP' AND
                  ila.lotAtt04 IN ('SETA', 'SET A', 'SET_A', 'SET-A', 'NONSET', 'NON SET', 'NON_SET', 'NON-SET') AND
      ...
      FROM ACT_TRANSACTION_LOG atl
        ...
        LEFT JOIN BSM_CODE BT
          ON BT.organizationId = atl.organizationId
          AND BT.codeType = 'BILLING_TRANSACTION_CATEGORY'
          AND CASE WHEN atl.tocustomerId = 'PPG' THEN BT.outerCode = atl.udf05 ELSE BT.outerCode = ila.lotAtt07 END
        LEFT JOIN BAS_LOCATION bl
          ON bl.organizationId = atl.organizationId
          AND bl.warehouseId = atl.warehouseId
          AND bl.locationId = atl.toLocation
      ...
      OR btd.billingTranCategory = ''
      OR btd.billingTranCategory = BT.codeid
      OR btd.billingTranCategory = atl.udf05  -- for PPG palletize & loose
      )
      AND btr.rate > 0
      AND NOT EXISTS (SELECT
          1
      ...
      ELSE
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHISTD_NW', IN_trans_no, 'no error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
...
--
-- Create procedure `CML_BILLHISTD_TYPE2_PICKPERASN_NW`
--
CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLHISTD_TYPE2_PICKPERASN_NW (IN IN_organizationId varchar(30),
IN IN_warehouseId varchar(30),
IN IN_USERID varchar(30),
IN IN_Language varchar(30),
IN IN_CustomerId varchar(30),
  ...
  START TRANSACTION;
    ...
    BEGIN
      ...
      SELECT
        ...
        -- Calculate billing quantity based on rate base
        CASE WHEN btd.ratebase = 'PALLET' THEN 1 ELSE 0 END AS qtyChargeBilling,
        0 AS qtyChargeTotDO,
        COUNT(atl.docLineNo) AS qtyChargeTotLine,
        0 AS totalCube,
        '' AS addTime,
      ...
      IF v_row_count = 0 THEN
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHISTD_PP_NW', IN_trans_no, 'error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
      ELSE
        -- Calculate execution time
        SET v_end_time = NOW();
        ...
        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLHISTD_PP_NW', IN_trans_no, 'no error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
...
--
-- Create procedure `Z_SP_ProcessBillingCML_BILLHIPICKPERASN`
--
CREATE
DEFINER = 'mysql.sys'@'%'
PROCEDURE Z_SP_ProcessBillingCML_BILLHIPICKPERASN ()
    SP:
  BEGIN
    -- Deklarasi variabel untuk menyimpan data dari cursor
    DECLARE v_organizationId varchar(20);
    ...
    AND zbcc.active = 'Y'
    AND zbccd.active = 'Y'
    AND zbccd.spName = 'CML_BILLHISTD_TYPE2_PICKPERASN_NW'
    AND DATE(dahu.closeTime) > getBillFMDate(25)
    AND NOT EXISTS (SELECT
        1
      FROM BIL_SUMMARY bs
    ...
    LOOP
      ...
      SET @IN_CustomerId = v_customerId;
      SET @IN_trans_no = v_trans_no;
      CALL CML_BILLHISTD_TYPE2_PICKPERASN_NW(@IN_organizationId, @IN_warehouseId, @IN_USERID, @IN_Language, @IN_CustomerId, @IN_trans_no, @p_success_flag, @p_message, @p_record_count);
...
CREATE
DEFINER = 'root'@'localhost'
PROCEDURE CML_BILLASNVASSTD_NW (IN IN_organizationId varchar(30),
IN IN_warehouseId varchar(30),
IN IN_USERID varchar(30),
IN IN_Language varchar(30),
IN IN_CustomerId varchar(30),
  ...
  START TRANSACTION;
    ...
    BEGIN
      ...
      SELECT
        ...
        bil.chargeCategory,
        bil.chargeType,
        CASE WHEN dah.customerId = 'PPG' AND
            dah.warehouseId = 'CBT01' AND
            dav.vasType IN ('PW', 'SWR', 'WR') THEN 0 ELSE bil.rate END AS rate,
        -- bil.rate,
        'QUANTITY' AS ratebase,
        0 cost,
        1 AS rateperunit,
      ...
      FROM DOC_ASN_VAS dav
        ...
        INNER JOIN Z_BAS_CUSTOMER_CUSTBILLING_DETAILS zbccd
          ON (zbcc.organizationId = zbccd.organizationId
          AND zbcc.lotatt01 = zbccd.idGroupSp)
      WHERE dav.organizationId = IN_organizationId
      AND dav.warehouseId = IN_warehouseId
      AND dah.customerId = IN_CustomerId
      AND DATE(dahu.closeTime) >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)
      AND zbcc.lotatt01 <> ''
      AND zbcc.active = 'Y'
      ...
      ELSE
        ...
        -- Set success output parameters
        SET p_success_flag = 'Y';
        SET p_message = CONCAT('VasASN billing processed successfully. ',
        'Billing Summary ID: ', v_billing_summary_id,
        '. Records: ', v_row_count,
        '. Execution time: ', v_execution_time, ' seconds');
        SET p_record_count = v_row_count;

        CALL Z_RECORD_SPBILLINGLOG(IN_warehouseId, IN_CustomerId, 'CML_BILLASNVASSTD_NW', IN_trans_no, 'no error', NULL, NULL, p_success_flag,
        p_message, p_record_count, v_start_time, v_end_time);
...
CREATE
DEFINER = 'root'@'%'
PROCEDURE CML_BILLFIXCHG_NW (OUT p_success_flag char(1),
OUT p_message varchar(1000),
OUT p_record_count int)
BEGIN
  -- =============================================
  ...
  START TRANSACTION;
    ...
    BEGIN
      ...
      -- Generate billing summary ID
      SET v_return_code = '*_*';
      CALL SPCOM_GetIDSequence_NEW('OJV_CML',
      '*',
      'en',
      'BILLINGSUMMARYIDCUST',
      v_billing_summary_id,
      ...
      containerType, containerSize)
        ...
        FROM (SELECT DISTINCT
            ...
            DAY(STR_TO_DATE(CONCAT(YEAR(NOW()), '-', MONTH(NOW()), '-', bth.udf02),
            '%Y-%m-%d')) AS billingDate,
      ...
      ELSE
        ...
        SELECT
          zcml_alert_message(p_message, '-4075819187');
      END IF;
