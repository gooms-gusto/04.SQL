...
DROP PROCEDURE OJV_CML_SPUDF_Process1;
    ...
    LOOP
      ...
      IF r_arrivedata > r_udf01
        ...
        IF r_docType = 'ASN'
          ...
          AND A.warehouseId = IN_warehouseId
          AND A.asnNo = r_docNo
          AND A.customerid NOT IN ('PPG', 'GMC', 'GCM', 'GYI', 'MAP', 'PSLOVE', 'YFI', 'MDS', 'NLDC', 'ECCOSBY', 'FMI_JKT', 'PANDA', 'TUMI', 'LTL_SMG')
          AND A.asnType <> 'CROSS';
        END IF;
        IF r_docType = 'LOAD'
        ...
        IF r_docType = 'SO'
          ...
          AND A.orderno = r_docNo
          AND A.orderType <> 'CROSS'
          AND A.customerid NOT IN ('PPG', 'GMC', 'GCM', 'GYI', 'MAP', 'ECCOSBY', 'FMI_JKT', 'PANDA', 'TUMI')
          AND A.warehouseId NOT IN ('SBYKK')
