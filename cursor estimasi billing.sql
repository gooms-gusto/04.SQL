...
INSERT INTO prod_apibilling.bt_estimasi_storage
(
 organtizationId
 ,warehouseId
 ,customerId
...
SELECT  'OJV_CML'AS organtizationId,HT.warehouseId,HT.CustomerID,HT.salesoffice, HT.divisionNo,max(HT.qtyTrace) as TotalPallet,HT.R_rate,(max(HT.qtyTrace)) * HT.R_rate AS TotalAmount, NOW() AS addDate,
NOW() AS editTime,'TIMER' AS addWho
FROM (
SELECT
                zib.organizationId,
                zib.warehouseId,
   ...
   HT
    ...
    HT.CustomerID,
    HT.R_rate,
HT.salesoffice, HT.divisionNo;


-- SELECT * FROM bt_estimasi_storage bes;

-- SELECT * FROM reporting_wms.BAS_CUSTOMER bc WHERE bc.customerType='OW' AND bc.activeFlag='Y'



-- 
-- SELECT DATE_FORMAT(DATE_ADD(NOW(), INTERVAL 7 HOUR)- interval 1 DAY,"%Y%m%d");
-- 
-- SELECT DATE_FORMAT(billingDate, "%Y-%m-%d") FROM reporting_wms.BIL_TARIFF_HEADER WHERE  tariffId='BIL00315';
