...
INNER JOIN (
  ...
  AND tbs.StockDate >= DATE_FORMAT(DATE_ADD(STR_TO_DATE(CONCAT(YEAR(CURDATE()), '-', MONTH(CURDATE()), '-',  DAY(bth.billingdate)), '%Y-%m-%d'), INTERVAL -1 MONTH), '%Y-%m-%d')
  AND tbs.StockDate <= DATE_FORMAT(DATE_ADD(STR_TO_DATE(CONCAT(YEAR(CURDATE()), '-', MONTH(CURDATE()), '-',  DAY(bth.billingdate)), '%Y-%m-%d'), INTERVAL -1 DAY), '%Y-%m-%d')
    )
