...
DECLARE ln_RecType varchar(21)
  ...
  DECLARE line_done int DEFAULT FALSE;
  
  
  DECLARE cur_Orderline CURSOR FOR
  ...
  -- SELECT ORDER
   SELECT
            ...
            AND DATE_FORMAT(atl.addTime, '%Y-%m-%d') >= '2023-09-11'
  ...
  AND bth.tariffMasterId=R_tariffMasterId
  ...
  getTariff:
    LOOP
      ...
      BEGIN
        ...
        IF R_docType = ln_docType THEN
        BEGIN

          CALL debug_msg(@enabled, 'proceed ' + ln_docType);
        ...
        --  BEGIN
            ...
            , ediErrorCode2, ediErrorMessage2, billingTranCategory, orderType, containerType, containerSize)
              SELECT
                ...
                ln_grossWeight,
                R_rate,
                ln_qtyCharge * R_rate / R_rateperunit,
                (ln_qtyCharge * (R_rate / R_rateperunit)) + (ln_qtyCharge * (R_rate / R_rateperunit)) * R_INCOMETAX,
                0,
                R_cost * ln_qtyCharge,
                0,
                NOW() confirmTime,
                ...
                0 cosTax,
                ln_qtyCharge * R_rate / R_rateperunit incomeWithoutTax,
                0 cosWithoutTax,
                '' costInvoiceType,
                '' noteText,
      ...
      --    END;
        --  END IF; -- end if tmp bil sumary info2
