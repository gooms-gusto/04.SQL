SELECT
  ...
  a.muid,
  a.qty AS qty_each,
  (a.qty/ CASE WHEN IFNULL(l13p.qty, 0) != 0 THEN l13p.qty ELSE IFNULL(t.qty, 1) END) AS qty_ip ,
   (a.qty/ CASE WHEN IFNULL(l12p.qty, 0) != 0 THEN l12p.qty ELSE IFNULL(t.qty, 1) END) AS qty_cs ,
  a.qty / (
  ...
  a.qtyAllocated AS qtyAllocated_each,
    a.qtyAllocated / (
  CASE WHEN IFNULL(l13p.qty, 0) != 0 AND
      IFNULL(
      CASE WHEN IFNULL(bsm.reportUom, '') != '' THEN bsm.reportUom ELSE bas_sku.reportUom END,
      '') = 'CS' THEN l13p.qty ELSE IFNULL(t.qty, 1) END) AS qtyAllocatedcs,
  a.qtyAllocated AS qtyAllocated_cs,
  (a.qtyOnHold + 0) / (
  CASE WHEN IFNULL(l12p.qty, 0) != 0 AND
      IFNULL(
      CASE WHEN IFNULL(bsm.reportUom, '') != '' THEN bsm.reportUom ELSE bas_sku.reportUom END,
  ...
  a.qty / (
  CASE WHEN IFNULL(l13p.qty, 0) != 0 THEN l13p.qty ELSE IFNULL(t.qty, 1) END) -
  a.qtyAllocated /
  (
  CASE WHEN IFNULL(l13p.qty, 0) != 0 THEN l13p.qty ELSE IFNULL(t.qty, 1) END) -
  (a.qtyOnHold +
  ...
  a.qtyMvOut /
  (
  CASE WHEN IFNULL(l13p.qty, 0) != 0 THEN l13p.qty ELSE IFNULL(t.qty, 1) END) AS qty_available_ip,
  a.qty - a.qtyAllocated - (a.qtyOnHold + 0) - a.qtyRpOut - a.qtyMvOut AS qty_available_ea,
  (SELECT
      IFNULL(SUM(qtyPicked_each), 0)
    FROM DOC_ORDER_DETAILS dd
    WHERE a.organizationId = dd.organizationId
    AND dd.warehouseId = a.warehouseId
    AND dd.sku = a.sku
    AND dd.lineStatus = '60') AS QtyPickedEA,
  (SELECT
      IFNULL(SUM(qtyPicked_each), 0) / CASE WHEN IFNULL(l12p.qty, 0) != 0 THEN l12p.qty ELSE IFNULL(t.qty, 1) END
    FROM DOC_ORDER_DETAILS dd
    WHERE a.organizationId = dd.organizationId
    AND dd.warehouseId = a.warehouseId
    AND dd.sku = a.sku
    AND dd.lineStatus = '60') AS QtyPickedCS,
  ...
  ila.lotAtt06,
  ila.lotAtt07,
  CASE WHEN ila.lotAtt08 = 'Y' THEN 'DAMAGE' WHEN ila.lotAtt08 = 'N' THEN 'OK' ELSE 'UNKNOWN' END AS lotAtt08,
  ila.lotAtt09,
  ila.lotAtt10,
  ila.lotAtt11,
  ila.lotAtt12,
...
AND a.warehouseId = 'SMG-SO'
AND a.customerId = ('NLDC')
--         AND a.sku        IN('${sku}')
