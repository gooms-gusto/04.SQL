...
SELECT
CONCAT('Rp', FORMAT( SUM(CUAN.BILLING_AMOUNT) , 2))
FROM
(
...
LEFT JOIN BSM_WAREHOUSE bw ON bs.organizationId = bw.organizationId AND bs.warehouseId = bw.warehouseId
 ...
 (DATE(bs.billingfromdate) >'2025-04-25'
 AND DATE(bs.billingfromdate) < '2025-05-26')
 ELSE
 (DATE(bs.billingfromdate) >'2025-04-25'
 AND DATE(bs.billingfromdate) < '2025-05-26')
 END
...
--  AND bs.warehouseid NOT IN ('SBYMM','BASF02','BASF01')
 ...
 GROUP BY bs.warehouseid,bw.udf02,btm.udf01,bc.udf02) CUAN;


SELECT * FROM BIL_TARIFF_MASTER btm WHERE btm.organizationId='OJV_CML';