...
DROP PROCEDURE IF EXISTS CML_EXECBILLHI;

DELIMITER $$

CREATE
DEFINER = 'wms_ftest'@'%'
PROCEDURE CML_EXECBILLHI (IN IN_bizOrgId varchar(20),
...
BEGIN
  ...
  WHERE cmc.udf01='N' AND cmc.transactionType='ASN';
  ...
  LOOP
    ...
    IF EXISTS (SELECT
      ...
      IF LENGTH(l_spname) > 0 THEN
        ...
        WHERE docNo=l_docNo AND warehouseId=l_warehouseId AND CustomerId=l_customerId AND transactionType='ASN';
