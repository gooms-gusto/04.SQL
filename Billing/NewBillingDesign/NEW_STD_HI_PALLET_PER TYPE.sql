...
BEGIN
  ...
  SELECT
    ...
    IFNULL(CAST((SUM(aad.qtyShipped_each/bpd1.qty)) AS char(255)), 0) AS qtyCharge,
...
cur_order_loop:
  LOOP
    ...
    od_organizationId, od_orderNo, od_soReference1, od_soReference3, od_orderType, od_docType, od_docTypeDescr, od_soStatus, od_warehouseId,
    ...
    od_grossWeight, od_cubeNya, od_tariffMasterId, od_QtyPerCases, od_QtyPerPallet, od_zone, od_batch, od_lotAtt07,od_Billtranctg, od_RecType;
  ...
  BLOCK2:
    ...
    getTariff:
      LOOP
        ...
        #
        -- SELECT CONCAT("masuk ",order_done," ",R_docType," ",od_orderType) ;

        --	SELECT od_orderType,R_docType;
      --  SELECT od_orderType,R_docType,od_Billtranctg,R_BILLINGTRANCATEGORY;
        IF (od_orderType = R_docType) AND (od_Billtranctg=R_BILLINGTRANCATEGORY) THEN
