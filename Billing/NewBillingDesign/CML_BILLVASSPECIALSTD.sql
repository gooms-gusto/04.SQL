...
DROP PROCEDURE IF EXISTS CML_BILLVASSPECIALSTD;
...
cur_order_loop:
  ...
  BLOCK2:
    BEGIN
      ...
      AND btd.chargeCategory = 'VA' AND btd.udf01 IN ('1700000145') 
      AND btr.rate > 0 AND btd.tariffLineNo > 100  
    ...
    getTariff:
      LOOP
        ...
        SELECT
          UPPER(R_CHARGECATEGORY),UPPER(R_CHARGETYPE),UPPER(od_chargeCategory),UPPER(od_chargeType);

        IF (UPPER(R_CHARGETYPE) = UPPER(od_chargeType)) AND (UPPER(R_CHARGECATEGORY)=UPPER(od_chargeCategory)) THEN
          -- CALCULATION
          -- UOM
          -- PALLET
          -- CUBIC
