...
BEGIN
  ...
  SELECT
    ...
    IFNULL(CAST(SUM(aad.qtyShipped_each / bpdCS.qty) AS char(255)), 0) AS qtyChargeCS,
    IFNULL(CAST(SUM(aad.qtyShipped_each / bpdIP.qty) AS char(255)), 0) AS qtyChargeIP,
    IFNULL(CAST(SUM(aad.qtyShipped_each / bpdPL.qty) AS char(255)), 0) AS qtyChargePL,
...
cur_order_loop:
  LOOP
    ...
    od_organizationId, od_orderNo, od_soReference1, od_soReference3, od_orderType, od_docType,
    ...
    od_QtyPerPallet, od_zone, od_batch, od_lotAtt07, od_Billtranctg;
  ...
  BLOCK2:
    ...
    getTariff:
      LOOP
        ...
        SELECT
          configValue INTO R_PALLETCNT
        FROM BSM_CONFIG_RULES bcm
        WHERE bcm.configId = 'PLT_CNT'
        AND bcm.warehouseId = R_WAREHOUSEID
        AND bcm.customerId = R_CUSTOMERID;
        ...
        IF (od_orderType = R_docType)
          AND (od_Billtranctg = R_BILLINGTRANCATEGORY) THEN
