      ...
      getTariff:
        LOOP
          ...
          IF (LTRIM(RTRIM(R_WAREHOUSEID)) = LTRIM(RTRIM(v_warehouseId)))
            AND (LTRIM(RTRIM(R_CUSTOMERID)) = LTRIM(RTRIM(v_customerId))) AND (LTRIM(RTRIM(R_chamberName)) = LTRIM(RTRIM(v_workingArea))) THEN
            -- CALCULATION
            -- UOM
            -- PALLET
            -- CUBIC
            ...
            , ediErrorCode2, ediErrorMessage2, billingTranCategory, orderType, containerType, containerSize)
              SELECT
                ...
                R_chargetype,
                -- R_descrC,
                CONCAT('Storage ', v_workingArea, ' ', v_warehouseId),
                R_rateBase,
                R_rateperunit,
                v_qtyCharge,
                '',
