  ...
  FROM
        ...
        WHERE
         ...
         -- AND date(zib.StockDate) = date(DATE_ADD(NOW(), INTERVAL - 1 DAY))
            AND date(zib.StockDate) > '2024-10-25' AND date(zib.StockDate) < '2024-11-17'
        ORDER BY zib.customerId
        ) ls_storage
  GROUP BY ls_storage.warehouseId,
             ls_storage.customerId,
           ls_storage.StockDate
  ORDER BY ls_storage.customerId;
...
read_loop:
  LOOP
    ...
    FETCH my_cursor INTO v_stockDate, v_warehouseId,v_customerId, v_qtyCharge;
    ...
    BLOCK2:
      ...
      getTariff:
        LOOP
          ...
          R_ratebase, R_ratePerUnit, R_rate, R_minAmount, R_maxAmount,R_qtyMinimumContract, R_minQty, R_materialNo, R_itemChargeCategory, R_billMode, R_LOCATIONCAT, R_LOCATIONGROUP, R_UDF06, R_UDF07, R_UDF08,
          ...
          IF (LTRIM(RTRIM(R_WAREHOUSEID)) = LTRIM(RTRIM(v_warehouseId))) AND
          (LTRIM(RTRIM(R_CUSTOMERID)) = LTRIM(RTRIM(v_customerId)))
          THEN
            -- CALCULATION
            -- UOM
            -- PALLET
            ...
            SELECT 'DEBUG=>', v_customerId,R_CUSTOMERID,R_WAREHOUSEID,v_warehouseId,R_RESULTQTYCHARGE;
            ...
            , ediErrorCode2, ediErrorMessage2, billingTranCategory, orderType, containerType, containerSize)
              SELECT
                ...
                R_chargetype,
                -- R_descrC,
                CONCAT('Storage ',v_customerId,' ',v_warehouseId),
                R_rateBase,
                R_rateperunit,
                v_qtyCharge,
                '',
