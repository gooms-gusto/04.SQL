    ...
    WHERE zib.organizationId = IN_organizationId
     AND zib.warehouseId IN ('CBT02', 'CBT03', 'LADC01')
      --   AND DATE(zib.StockDate) = DATE(DATE_ADD(NOW(), INTERVAL -1 DAY))
         ...
         AND DATE(zib.StockDate) < '2024-11-20'
...
UNION ALL
SELECT DATE(DATE_ADD(NOW(), INTERVAL -1 DAY)) AS stockDate,
 bth.warehouseId,
  btm.customerId,
 btr.udf03 AS workingArea,
0 AS qtycbm
FROM BIL_TARIFF_HEADER bth INNER JOIN BIL_TARIFF_DETAILS btd ON bth.organizationId = btd.organizationId
AND bth.warehouseId = btd.warehouseId AND bth.tariffId = btd.tariffId 
INNER JOIN BIL_TARIFF_RATE btr ON bth.organizationId = btr.organizationId AND bth.warehouseId = btr.warehouseId
AND bth.tariffId = btr.tariffId AND btd.tariffLineNo = btr.tariffLineNo
INNER JOIN BIL_TARIFF_MASTER btm ON bth.organizationId = btm.organizationId
AND bth.tariffMasterId = btm.tariffMasterId
WHERE bth.organizationId=IN_organizationId AND btd.warehouseId IN ('CBT02','CBT03','LADC01')  AND btm.Customerid=IN_CustomerId
  AND bth.effectiveFrom <= DATE_FORMAT(CURDATE(), '%Y-%m-%d')
        AND bth.effectiveTo >= DATE_FORMAT(CURDATE(), '%Y-%m-%d')
        AND btd.chargeCategory = 'IV'
        AND btr.rate > 0
        AND btr.udf03 NOT IN

(
SELECT DISTINCT CASE WHEN bl.warehouseId='CBT02' THEN 'LINC' ELSE bl.workingArea END AS workingArea
FROM
Z_InventoryBalance  lc INNER JOIN BAS_LOCATION bl ON lc.organizationId = bl.organizationId AND lc.warehouseId = bl.warehouseId AND lc.locationId = bl.locationId
WHERE lc.organizationId=IN_organizationId AND DATE(lc.StockDate) = DATE(DATE_ADD(NOW(), INTERVAL -1 DAY)) 
AND bl.warehouseId IN ('CBT02', 'CBT03', 'LADC01') AND lc.customerId=IN_CustomerId AND lc.qtyavailable > 0)
