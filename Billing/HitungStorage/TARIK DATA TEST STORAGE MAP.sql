...
UNION ALL
SELECT DATE(DATE_ADD(NOW(), INTERVAL -2 DAY)) AS stockDate,
 bth.warehouseId,
  btm.customerId,
...
WHERE bth.organizationId='OJV_CML' AND btd.warehouseId IN ('CBT02', 'CBT03', 'LADC01')  AND btm.Customerid='MAP'
...
WHERE lc.organizationId='OJV_CML' AND DATE(lc.StockDate) = DATE(DATE_ADD(NOW(), INTERVAL -2 DAY)) 
AND bl.warehouseId IN ('CBT02', 'CBT03', 'LADC01') AND lc.customerId='MAP' AND lc.qtyavailable > 0)
