 SELECT
  ...
  FROM (SELECT
      ...
      CASE WHEN zib.warehouseId = 'CBT02' THEN 'LINC' WHEN
      zib.warehouseId = 'CBT03' AND  l.workingArea='WHTMLC' THEN 'WHTMLD'
      ELSE l.workingArea END AS workingArea,
      zib.SKU AS sku,
    ...
    AND DATE(zib.StockDate) = DATE(DATE_ADD(NOW(), INTERVAL -2 DAY))
  ...
  GROUP BY ls_storage.warehouseId,
           ...
           ls_storage.workingArea
  ORDER BY ls_storage.customerId, stockDate;

UNION ALL
SELECT 
DATE(DATE_ADD(NOW(), INTERVAL -2 DAY)) AS stockDate,
'LADC01' AS warehouseid 
 FROM BIL_TARIFF_RATE btr INNER JOIN BIL_TARIFF_DETAILS btd
 ON btr.organizationId = btd.organizationId AND btr.warehouseId = btd.warehouseId
 AND btr.tariffId = btd.tariffId AND btr.tariffLineNo=btd.tariffLineNo  INNER JOIN BIL_TARIFF_HEADER  bth ON btr.organizationId = bth.organizationId
 AND btd.warehouseId = bth.warehouseId AND btd.tariffId = bth.tariffId
 WHERE btd.chargeCategory='IV' AND bth.tariffMasterId='BIL0006202';
...
SELECT  btr.udf03,btd.tariffId,bth.tariffMasterId,bth.warehouseId
FROM BIL_TARIFF_RATE btr INNER JOIN BIL_TARIFF_DETAILS btd
ON btr.organizationId = btd.organizationId AND btr.warehouseId = btd.warehouseId
AND btr.tariffId = btd.tariffId AND btr.tariffLineNo=btd.tariffLineNo  INNER JOIN BIL_TARIFF_HEADER  bth ON btr.organizationId = bth.organizationId
AND btd.warehouseId = bth.warehouseId AND btd.tariffId = bth.tariffId
INNER JOIN BIL_TARIFF_MASTER btm ON bth.organizationId=btm.organizationId AND bth.tariffMasterId = btm.tariffMasterId
WHERE btd.chargeCategory='IV' AND btm.customerId='MAP' AND bth.warehouseId='LADC01'
AND bth.effectiveFrom <= DATE_FORMAT(CURDATE(), '%Y-%m-%d')
        AND bth.effectiveTo >= DATE_FORMAT(CURDATE(), '%Y-%m-%d')
        AND btr.udf03 NOT IN (SELECT DISTINCT bl.workingArea 
        FROM INV_LOT_LOC_ID  zib INNER JOIN BAS_LOCATION bl ON zib.organizationId = bl.organizationId AND zib.warehouseId = bl.warehouseId 
        AND zib.locationId = bl.locationId WHERE bl.organizationId=bth.organizationId AND bl.warehouseId=bth.warehouseId AND zib.customerId=bth.Customerid
        AND DATE(zib.StockDate) = DATE(DATE_ADD(NOW(), INTERVAL -2 DAY)))
...
-- 
-- 
-- SELECT * FROM  BIL_TARIFF_RATE btr WHERE btr.tariffId='BIL00805'
