...
--
-- Drop procedure `CML_BILLHOSTD_TYPE2_NW`
--
DROP PROCEDURE IF EXISTS CML_BILLHOSTD_TYPE2_NW;
    ...
    BEGIN
      ...
      SELECT
        ...
        CASE WHEN btd.ratebase = 'DO' AND
            aad.customerId IN ('ECMAMAB2C') THEN CEIL(SUM(aad.qty_each / btr.classTo)) WHEN btd.ratebase = 'DO' AND
            aad.customerId NOT IN ('ECMAMAB2C') THEN COUNT(DISTINCT doh.orderNo) WHEN btd.ratebase = 'DID' THEN COUNT(DISTINCT aad.dropId) WHEN btd.ratebase = 'DID1' THEN COUNT(DISTINCT aad.dropId) ELSE 0 END AS qtyChargeBilling,
