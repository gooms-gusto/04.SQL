    ...
    getTariff:
      LOOP
        ...
        IF (UPPER(R_CHARGETYPE) = UPPER(od_chargeType))
          AND (UPPER(R_CHARGECATEGORY) = UPPER(od_chargeCategory)) THEN
          -- CALCULATION
          -- UOM
          -- PALLET
          -- CUBIC
    ...
    getTariff:
      LOOP
        ...
        IF (UPPER(R_VASTYPE) = UPPER(od_vasType)) THEN
          ...
          SET od_qtyCharge = R_RESULTQTYCHARGE;
...
BEGIN
  ...
  SELECT
    ...
    sumso.soReference1,
    sumso.soReference3,
    sumso.tariffMasterId,
    SUM(sumso.qtyChargeEA),
    SUM(sumso.qtyChargeCS),
    SUM(sumso.qtyChargeIP),
    SUM(sumso.qtyChargePL),
    SUM(sumso.qtyChargeCBM),
    SUM(sumso.qtyChargeTotDO),
    SUM(sumso.qtyChargeTotLine),
    SUM(sumso.qtyChargeNettWeight),
    SUM(sumso.qtyChargeGrossWeight),
    SUM(sumso.qtyChargeMetricTon),
    sumso.closeTime
  FROM (SELECT
    ...
    AND aad.sku NOT IN ('YFI001', 'DEMO TABLE AT', 'DEMO TABLE SAM', 'HOT STAMP', 'COLLATERAL-COLLECTABLE STICKER', 'HELLOBASTC', 'MAMALOVSTC', 'MAMANOUSTC', 'MAMAPCK02',
    'MAMASTT', 'STCMCBMS-TH', 'BUMILTSTC2', 'COLLATERAL-STICKERPINK', 'PRINCTSTC3') /*AB-update sku take out base on handing out ls-11-09-24*/
...
BEGIN
  ...
  AND atl.toSku NOT IN ('DEMO TABLE AT', 'DEMO TABLE SAM', 'HOT STAMP', 'COLLATERAL-COLLECTABLE STICKER', 'HELLOBASTC', 'MAMALOVSTC', 'MAMANOUSTC', 'MAMAPCK02',
