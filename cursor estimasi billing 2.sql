CREATE DEFINER=`akbar`@`%` PROCEDURE `CreateDailyEstimationStorageFlux`(
	OUT txtmessage varchar(4000)
)
BEGIN
...
-- process insert
SELECT OutCUSTOMERID,OutCURRENT_FM_BILLINGDATE,OutCURRENT_TO_BILLINGDATE;
...
SELECT
             ...
             -- AND zib.warehouseId = 'CBT01'
                AND  DATE_FORMAT(zib.StockDate,"%Y-%m-%d") >= OutCURRENT_FM_BILLINGDATE
              AND  DATE_FORMAT(zib.StockDate,"%Y-%m-%d") <= OutCURRENT_TO_BILLINGDATE
              AND zib.customerId IN (OutCUSTOMERID)   -- SELECT bc.customerId FROM BAS_CUSTOMER bc WHERE bc.activeFlag='Y' AND bc.customerType='OW')
              AND (ila.lotAtt07 = 'R'
              OR 'ST' <> 'PL')
              AND DATE_FORMAT(DATE_ADD(NOW(), INTERVAL 7 HOUR)- interval 1 DAY,"%Y-%m-%d") between DATE_FORMAT(DATE(bth.effectiveFrom),"%Y-%m-%d") and DATE_FORMAT(DATE(bth.effectiveTo),"%Y-%m-%d")
              AND btd.chargeCategory = 'IV'
