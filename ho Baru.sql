...
BEGIN
  ...
  SELECT
    ...
    df.closeTime,aad.allocationDetailsId AS line_trans -- add transaction line
...
IF NOT EXISTS (SELECT 1 FROM BIL_SUMMARY bs WHERE bs.organizationId=od_organizationId AND bs.warehouseId=od_warehouseId AND  bs.docNo= od_orderNo AND bs.udf03=od_line_transaction AND bs.chargeCategory='OB') THEN
