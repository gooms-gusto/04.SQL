SELECT DISTINCT
        doh.customerId                                                                 AS CUSTOMER_CODE ,
          DATE_FORMAT(dlh.loadingToTime, '%Y-%m-%d')    AS loaddate2  ,
        'DC18' AS Source,
        'SAMSONITE' AS Brand,
'' AS CategoryGroup,
          arh.vehicleNo                                                        AS TRUCK_NUMBER   ,
     arh.carrierId AS EXPEDITION_ID,
        ca.carrierName                                                                AS EXPEDITION_NAME,
        '' AS VIA,
...
ON
  ...
  LEFT OUTER JOIN
        (
                ...
                SELECT organizationId,customerId,customerDescr1 AS carrierName FROM BAS_CUSTOMER WHERE customerType = 'CA' LIMIT 1
        )
        ca
ON
