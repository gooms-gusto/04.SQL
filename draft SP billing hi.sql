...
--                       '|Cust:', IN_CustomerId, '|Trans:', IN_trans_no));
    ...
    IF IN_organizationId IS NULL OR IN_organizationId = '' THEN
        SET p_message = 'Organization ID cannot be empty';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = p_message;
    END IF;
    
    IF IN_CustomerId IS NULL OR IN_CustomerId = '' THEN
        SET p_message = 'Customer ID cannot be empty';
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = p_message;
    END IF;
    
    IF IN_trans_no IS NULL OR IN_trans_no = '' THEN
