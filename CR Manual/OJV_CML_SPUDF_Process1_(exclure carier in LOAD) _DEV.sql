...
ENDPROC:
  BEGIN
    ...
    AND A.udf01 <> 'Y'
  --  AND E.ldlNo IN ('LDL2410300002','LDL2410300001')
...
-- SELECT
--         EXTRACT(HOUR_MINUTE FROM r_arriveTime);
      ...
      IF r_arrivedata > r_udf01
        ...
        IF r_docType = 'ASN'
          ...
          AND A.warehouseId = IN_warehouseId
          AND A.asnNo = r_docNo
          AND A.customerid NOT IN ('PPG', 'GMC', 'GCM', 'GYI', 'MAP', 'PSLOVE', 'YFI', 'MDS', 'NLDC', 'ECCOSBY')
          AND A.asnType <> 'CROSS';
        END IF;
        IF r_docType = 'LOAD'
        ...
        IF r_docType = 'SO'
          ...
          AND A.orderno = r_docNo
          AND A.orderType <> 'CROSS'
          AND A.customerid NOT IN ('PPG', 'GMC', 'GCM', 'GYI', 'MAP', 'ECCOSBY')
          AND A.warehouseId NOT IN ('SBYKK')
          AND A.carrierId NOT IN ('KEDCA1522', 'LINC_EXP', '5000000027', 'JNE', '0000001794', '0000002529', '0000004182', 'BP', 'LINC-EXP', 'LINC-X', '', 'ECCOSBY');
        END IF;
        IF r_docType = 'ASN'
